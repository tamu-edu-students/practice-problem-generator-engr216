# spec/models/particle_statics_problem_generator_spec.rb
require 'rails_helper'

RSpec.describe ParticleStaticsProblemGenerator, type: :model do
  subject(:generator) { described_class.new('Particle Statics') }
  ...
  describe 'private method: generate_ps_problem_from_data' do
    context "when input_type is 'fill_in' and answer is a numeric string" do
      let(:data) do
        {
          question: 'Calculate tension in a rope',
          answer: '5.67',
          input_type: 'fill_in',
          field_label: 'Tension'
        }
      ...
      it 'creates a single text input field' do
        result = generator.send(:generate_ps_problem_from_data, data)
        expect(result[:input_fields]).to eq([
                                              { label: 'Tension', key: 'ps_answer', type: 'text' }
                                            ])
      end
    end
    ...
    context "when input_type is 'fill_in' and answer is an array of numeric strings" do
      ...
      it 'creates multiple text inputs for each numeric part' do
        result = generator.send(:generate_ps_problem_from_data, data)
        expect(result[:input_fields]).to eq([
                                              { label: 'Answer part 1', key: 'ps_answer_1', type: 'text' },
                                              { label: 'Answer part 2', key: 'ps_answer_2', type: 'text' }
                                            ])
      end
    end
    ...
    context "when input_type is 'fill_in' and answer is a non-numeric string" do
      ...
      it 'creates a radio input field with options' do
        result = generator.send(:generate_ps_problem_from_data, data)
        expect(result[:input_fields]).to eq([
                                              { label: 'Your Answer', key: 'ps_answer', type: 'radio',
    ...
    context "when input_type is 'multiple_choice'" do
      let(:data) do
        {
          question: 'Particle statics equation select',
          answer: 'A',
  ...
  describe 'static_questions' do
    it 'returns an array of 6 static questions with input_fields', :aggregate_failures do
      static_qs = generator.send(:static_questions)
      expect(static_qs).to be_an(Array)
      expect(static_qs.size).to eq(6)
      static_qs.each do |q|
