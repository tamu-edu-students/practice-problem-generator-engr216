...
module ErrorPropagationQuestionText
  ...
  def format_formula(formula)
    ...
    formatted = formatted.gsub(/([a-zA-Z\d]+)\/([a-zA-Z\d]+)/) do |match|
    ...
    formatted = formatted.gsub(/(\w+)\^?(\d+)/) do |match|
      base = $1
      exponent = $2
    ...
    # Format subscripts
    formatted = formatted.gsub(/(\w+)_(\w+)/, '\1<sub>\2</sub>')
  ...
  def multi_variable_template(physical_quantity, formula, var1, val1, unc1, unit1, var2, val2, unc2, unit2)
